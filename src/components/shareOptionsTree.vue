<template>
	<span class="tree-expand">
		<span class="codeName codeNameEdit tree-label" :style="'width:'+(250-NODE.level*18)+'px'" v-show="DATA.isEdit">
		...
		:class="'codeName ' + [DATA.id > maxexpandId ? 'tree-new tree-label' : 'tree-label']" :style="'width:'+(250-NODE.level*18)+'px'">
    ...
    <span class="codeExample" v-show="!DATA.isEdit" :style="'width:'+((!DATA.hasCodeValue || DATA.children)?'230px':'160px;')">
      {{DATA.codeExample?DATA.codeExample:"--"}}
    </span>
    ...
    <span class="codeValue" v-if="DATA.hasCodeValue && DATA.codeValue && !DATA.children" v-show="!DATA.isEdit">
      <el-dropdown>
...
<script>
    ...
    data (){
      return {
		...
		props: ['NODE', 'DATA', 'STORE', 'maxexpandId','saveApi'],
		methods: {
			nodeAdd(s,d,n){//新增
